...
public class ZGrid {
	...
	public ZGrid(int maxZoom) {
		...
		// maxZoom so that 8 Bit ZoomLevel + maxZoom Bits per Dimension(2) = 64
		// Bit
		// (Long)
		if (maxZoom > 28) {
	...
	public long getZIdLowerLeft(OSHDBBoundingBox bbox) {
		return getZIdLowerLeft(bbox.getMinLonLong(), bbox.getMaxLonLong(), bbox.getMinLatLong(), bbox.getMaxLatLong());
		
	}
	
	public long getZIdLowerLeft(long minLon, long maxLon, long minLat, long maxLat) {
		minLon = normalizeLon(minLon);
		maxLon = normalizeLon(maxLon);
		minLat = normalizeLat(minLat);
		maxLat = normalizeLat(maxLat);

		long dLon = maxLon - minLon;
		long dLat = maxLat - minLat;
		...
		int zoom = maxZoom;
		for (; zoom >= 0; zoom--) {
			double cellWidth = cellWidths[zoom];
			double cells = d / cellWidth;
			if (cells >= 2) {
				continue;
			}
			
			minX = (long) (minLon / cellWidth);
			minY = (long) (minLat / cellWidth);
			...
			// check
			long maxX = (long) ((maxLon - epsilon) / cellWidth);
			long maxY = (long) ((maxLat - epsilon) / cellWidth);
		...
		return addZoomToId(morton(minX, minY), zoom);
	}
	...
	public long getIdSingleZIdWithZoom(long minLon, long maxLon, long minLat, long maxLat) {
		...
		if (!validateLon(minLon) || !validateLon(maxLon) || !validateLat(minLat) || !validateLat(maxLat)) {
			return -1;
		}
	...
	public static long getParent(long zId, int parentZoom) {
		...
		if (zoom < parentZoom) {
			throw new IllegalArgumentException(
					"zoom of id already lesser than parentZoom (zoom:" + zoom + " parentZoom:" + parentZoom + ")");
		}
		if (zoom == parentZoom) {
			return zId;
		}
	...
	public int optimalZoom(long delta) {
		if (delta == 0) {
			return maxZoom;
		}
			...
			while (z > 0) {
				...
				if (states[z - 1].complete) {
					...
					case 0:
						...
						break;
					case 1:
						state.bbox = new OSHDBBoundingBox(parentBBox.getMinLonLong() + state.width,
								parentBBox.getMinLatLong(), parentBBox.getMinLonLong() + 2 * state.width,
								parentBBox.getMinLatLong() + state.width);
						break;
					case 2:
						state.bbox = new OSHDBBoundingBox(parentBBox.getMinLonLong(),
								parentBBox.getMinLatLong() + state.width, parentBBox.getMinLonLong() + state.width,
								parentBBox.getMinLatLong() + 2 * state.width);
						break;
					case 3:
						state.bbox = new OSHDBBoundingBox(parentBBox.getMinLonLong() + state.width,
								parentBBox.getMinLatLong() + state.width, parentBBox.getMinLonLong() + 2 * state.width,
								parentBBox.getMinLatLong() + 2 * state.width);
					}
